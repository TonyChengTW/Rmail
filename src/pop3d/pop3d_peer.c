...
/*	#include "pop3d.h"
/*
/*	void	pop3d_peer_init(state)
/*	POP3D_STATE *state;
/*
/*	void	pop3d_peer_reset(state)
/*	POP3D_STATE *state;
/* DESCRIPTION
/*	The pop3d_peer_init() routine attempts to produce a printable
/*	version of the peer name and address of the specified socket.
/*	Where information is unavailable, the name and/or address
/*	are set to "unknown".
/*
/*	pop3d_peer_init() updates the following fields:
/* .IP name
...
/* .RE
/* .PP
/*	pop3d_peer_reset() releases memory allocated by pop3d_peer_init().
/* LICENSE
/* .ad
/* .fi
...
/* pop3d_peer_init - initialize peer information */
...
void    pop3d_peer_init(POP3D_STATE *state)
{
	...
	state->peer_code = POP3D_PEER_CODE_PERM;
    }
	...
	if (hp == 0) {
	    ...
	    state->peer_code = (h_errno == TRY_AGAIN ?
				POP3D_PEER_CODE_TEMP : POP3D_PEER_CODE_PERM);
	    ...
	    state->peer_code = POP3D_PEER_CODE_OK;
...
#define REJECT_PEER_NAME(state, code) { \
	...
	state->peer_code = code; \
		...
		REJECT_PEER_NAME(state, POP3D_PEER_CODE_PERM);
	    } else {
		for (i = 0; /* void */ ; i++) {
...
/* pop3d_peer_reset - destroy peer information */
