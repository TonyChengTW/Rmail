...
/* Rmail need mysql header */
#include <mysql.h>

extern MYSQL mta_dbh;
extern MYSQL cac_dbh;
...
/* Rmail cache */
typedef struct HOSTMAP_ENTRY {
  unsigned int idx;
  char hostname[256];
  char nodename[256];
  struct HOSTMAP_ENTRY *next_entry;
} HOSTMAP_ENTRY;

extern HOSTMAP_ENTRY *hmap_cache;
extern int hmap_last;

typedef struct TRANSPORT_ENTRY {
  unsigned int idx;
  char domain[128];
  char basedir[128];
  struct TRANSPORT_ENTRY *next_entry;
} TRANSPORT_ENTRY;
extern TRANSPORT_ENTRY *tran_cache;
extern int tran_last;

extern char myhostname[64];
