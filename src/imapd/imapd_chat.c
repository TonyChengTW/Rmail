...
/*
/*	void	imapd_chat_query(state)
/*	IMAPD_STATE *state;
/*
/*	void	imapd_chat_reply(state, format, ...)
/*	IMAPD_STATE *state;
/*	char	*format;
/*
/*	void	imapd_chat_notify(state)
/*	IMAPD_STATE *state;
/*
/*	void	imapd_chat_reset(state)
/*	IMAPD_STATE *state;
/* DESCRIPTION
/*	This module implements POP3 server support for request/reply
/*	conversations, and maintains a limited POP3 transaction log.
/*
/*	imapd_chat_query() receives a client request and appends a copy
/*	to the POP3 transaction log.
/*
/*	imapd_chat_reply() formats a server reply, sends it to the
...
/*
/*	imapd_chat_notify() sends a copy of the POP3 transaction log
...
/*	imapd_chat_notify() when no POP3 transaction log exists.
/*
/*	imapd_chat_reset() resets the transaction log. This is
...
void    imapd_chat_reset(IMAPD_STATE *state)
{
    if (state->history) {
...
static void imap_chat_append(IMAPD_STATE *state, char *direction)
{
    char   *line;
...
/* imapd_chat_query - receive and record an POP3 request */

void    imapd_chat_query(IMAPD_STATE *state)
{
    int     last_char;

    last_char = imap_get(state->buffer, state->client, var_line_limit);
    imap_chat_append(state, "In:  ");
    if (last_char != '\n')
...
/* imapd_chat_reply - format, send and record an POP3 response */

void    imapd_chat_reply(IMAPD_STATE *state, char *format,...)
{
    va_list ap;
    int     delay = 0;
     ...
     */
    if (state->error_count >= var_imapd_soft_erlim)
	sleep(delay = var_imapd_err_sleep);

    imap_fputs(STR(state->buffer), LEN(state->buffer), state->client);
	...
	vstream_longjmp(state->client, IMAP_ERR_TIME);
    if (vstream_ferror(state->client))
	vstream_longjmp(state->client, IMAP_ERR_EOF);
}
...
/* imapd_chat_notify - notify postmaster */

void    imapd_chat_notify(IMAPD_STATE *state)
{
  /*
    char   *myname = "imapd_chat_notify";
    VSTREAM *notice;
    char  **cpp;
  */
    /*
