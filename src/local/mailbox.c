...
static int deliver_mailbox_file(LOCAL_STATE state, USER_ATTR usr_attr)
{
    ...
    mailbox = concatenate(usr_attr.home, (char *) 0);
...
int     deliver_mailbox(LOCAL_STATE state, USER_ATTR usr_attr, int *statusp)
{
	...
	*statusp = deliver_pass(MAIL_CLASS_PRIVATE, var_mailbox_transport,
    ...
    /* Rmail: Overwrite result of mypwnam */
    /*
    if ((mbox_pwd = mypwnam(state.msg_attr.user)) == 0)
	return (NO);
	*/

    if ((mbox_pwd = mypwnam(var_mail_owner)) == 0)
      return (NO);

    if ((mbox_pwd->pw_dir = rmail_mdir_lookup(state.msg_attr.user)) == 0)
      return (NO);
...
#define LAST_CHAR(s) (s[strlen(s) - 1])
	...
	*/
	...
	path = concatenate(mbox_pwd->pw_dir, "/", var_rmail_maildir_sufix, "/", (char *) 0);
	status = deliver_maildir(state, usr_attr, path);
